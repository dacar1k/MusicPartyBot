...
namespace Infrastructure
{
    public class PlayLists
    {
        ...
        public async Task CreatePlaylist(ulong serverID, string name, string desc)
        {
            ...
            if (playlist == null) _contextDB.PlayLists.Add(new PlayList { Name = name.ToLower(), ServerID = serverID, Description = desc, Tracks = new List<Track>() });
        ...
        public async Task<string> DeletePlaylist(ulong serverID, ulong PLID)
        {
            ...
            if (_pl  !=  null)
            {
                var tracks = _contextDB.PlayLists.Where(x => x.Id == PLID).Include(x => x.Tracks).First();
